...
gender-biased corpus yields $L_1 = 0.006$, $L_2 = 2.093$, $L_3 = 1.71 \times 10^{-5}$,
and $\mathrm{AS} = 122{,}292$: the model substantially redistributed probability mass across
...
\midrule
$L_1$ (mean KL) & $0.0063$ & Near-zero token-level shift \\
$L_2$ (mean KL) & $2.093$ & Large inter-group probability redistribution \\
...
Ratio $L_2/L_1$ & ${\approx}\,330$ & Systematic group-level shift, not token-level \\
$L_2$ std / max & $0.94$ / $4.47$ & Variable across contexts, consistently large \\
\bottomrule
\end{tabular}
\end{table}
...
The value $L_1 = 0.006$ is near zero: the relative distribution over the 14 target tokens
has barely changed between $M_0$ and $M_1$. Token-level lexical preferences are stable.

The value $L_2 = 2.093$ is large. To contextualise this magnitude: a KL divergence of 2
on a two-element distribution corresponds to a very substantial shift in the probability
ratio between the two groups. The standard deviation across contexts (0.94) and the
maximum per-context value (4.47) indicate that the effect is variable but consistently
present. The ratio $L_2/L_1 \approx 330$ confirms that the redistribution is systematic at
...
preserved. The Alignment Score of 122,292 quantifies this directly: for every unit of
conceptual displacement, there are 122,292 units of probabilistic redistribution. The model
...
Alignment Score of 122,292, indicating extreme shallow alignment: the model learned to
